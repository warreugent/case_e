...
void read_cyclic_roster()
{
    ...
    for (k = 0; k < number_nurses; k++) //elke rij is een nieuwe nurse
    {
        for (i = 0; i < number_days; i++) //elke kolom is een dag
        {
            fscanf(input, "%d\t", &l);
            cyclic_roster[k][i] = shift[l];//leest de value l uit de input file en steekt de waarde in array shift op index l in de cyclic roster[k][i]
        }
    }
...
void create_output_directory() {
    if (_mkdir("output") != 0) {  // Try to create the "output" directory
        if (errno != EEXIST) {  // Ignore if the directory already exists
            printf("Error creating output directory: %s\n", strerror(errno));
            exit(1);  // Exit if the folder can't be created
        }
    }
}
...
void print_output()
{
    ...
    /** Output is now written to the 'output' folder with the same filename */
    strcpy(filename, "output/Monthly_Roster_dpt_");  // Set the path to the 'output' folder
    strcat(filename, department);  // Append the department name to the filename
    strcat(filename, ".txt");  // Append ".txt" to the filename

    printf("Trying to open file: %s\n", filename);
    output = fopen(filename, "w");  // Open the file in write mode ("w" to overwrite)
    ...
    // Loop through all nurses and their respective shifts
    for (k = 0; k < number_nurses; k++)
    {
        fprintf(output, "%s\t", personnel_number[k].c_str());  // Write personnel number

        for (i = 0; i < number_days; i++)
        {
            shift_decoding(monthly_roster[k][i]);  // Decode the shift
            fprintf(output, "%d\t", a1);  // Write the decoded shift
        }
        fprintf(output, "\n");  // Move to the next line after each nurse
    }
...
{
    // Set the path to the 'output' folder for the violations file
    strcpy(filename, "output/Violations_dpt_");
    strcat(filename, department);  // Append the department name
    strcat(filename, ".txt");  // Append ".txt" to the filename
    ...
    // Initialize the scheduled array to 0 for all nurses, days, and shifts
    ...
    // Initialize the violations array to 0
    for (i = 0; i < 20; i++)
        violations[i] = 0;

    // Evaluate the work schedule for each nurse
    for (i = 0; i < number_nurses; i++) {
        evaluate_line_of_work();
    }

    // Output the evaluation results to the file
...
void procedure()
{
    ...
    // leave days should always be granted

    //write some methods

}

/*void initialise_rosters()
{
    roster1 = [][];
    for (k = 0; k < number_nurses; k++)			// Example: set your monthly roster exactly equal to the cyclic roster
    {
        for (i = 0; i < number_days; i++)
            roster1[k][i] = cyclic_roster[k][i];
    }
}*/
